package br.senai.sp.jandira.mytrips.repository

import android.content.Context
import br.senai.sp.jandira.mytrips.dao.MyTripsDb
import br.senai.sp.jandira.mytrips.model.Viagem
class ViagemRepository (context: Context){
    private val db = MyTripsDb.getBancoDeDados(context).viagensDao()

    fun salvar (viagem: Viagem):Long{
        return db.salvar(viagem)
    }
    fun atualizar (viagem: Viagem):Int{
        return db.atualizar(viagem)
    }
    fun excluir (viagem: Viagem):Int{
        return db.excluir(viagem)
    }

    fun listarTodosAsViagens():List<Viagem>{
        return db.listarTodasAsViagens()
    }
//    @Composable
//    fun listarTodasAsViagens():List<Viagem>{
//        val londres = Viagem(
//            id = 1,
//            destino = "Londres",
//            descricao = "Londres, capital da Inglaterra e do Reino Unido, é uma cidade do século 21 com uma história que remonta à era romana.",
//            dataChegada = LocalDate.of(2020, 1,15),
//            dataPartida = LocalDate.of(2020, 1,20),
//            imagem = painterResource(id = R.drawable.foto_londres)
//        )
//        val paris = Viagem(
//            id = 2,
//            destino = "Paris",
//            descricao = "Paris, a capital da França, é uma importante cidade europeia e um centro mundial de arte, moda, gastronomia e cultura.",
//            dataChegada = LocalDate.of(2018,12,18),
//            dataPartida = LocalDate.of(2018,12,26),
//            imagem = painterResource(id = R.drawable.foto_paris)
//        )
//        val porto = Viagem(
//            id = 3,
//            destino = "Porto",
//            descricao = "Porto é uma cidade costeira no noroeste de Portugal conhecida pelas pontes imponentes e pela produção de vinho do Porto.",
//            dataChegada = LocalDate.of(2016,10,5),
//            dataPartida = LocalDate.of(2019,10,17),
//            imagem = painterResource(id = R.drawable.foto_porto)
//        )
//        val curitiba = Viagem(
//            id = 4,
//            destino = "Curitiba",
//            descricao = "Curitiba é a capital do estado do Paraná, na região sul do Brasil.",
//            dataChegada = LocalDate.of(2019,4,1),
//            dataPartida = LocalDate.of(2019,4,15),
//            imagem = painterResource(id = R.drawable.foto_xangai)
//        )
//        val xangai = Viagem(
//            id = 5,
//            destino = "Xangai",
//            descricao = "Xangai, na costa central da China, é a maior cidade do país e um núcleo financeiro global.",
//            dataChegada = LocalDate.of(2024,2,7),
//            dataPartida = LocalDate.of(2024,2,14),
//            imagem = painterResource(id = R.drawable.foto_curitiba)
//        )
//
//        return listOf(londres, paris, porto, curitiba, xangai)
//    }
}